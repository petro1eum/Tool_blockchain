...
from trustchain import (
    TrustedTool, TrustLevel, SignatureAlgorithm,
    get_signature_engine, MemoryRegistry,
    create_integrated_security_system,
    enable_automatic_enforcement, disable_automatic_enforcement
)
from trustchain.monitoring.tool_enforcement import create_tool_enforcer
...
from trustchain.monitoring.hallucination_detector import HallucinationDetector, ValidationResult
from trustchain.monitoring.tool_enforcement import ToolExecutionEnforcer, ToolExecution
from trustchain.monitoring.tool_enforcement_interceptor import UnauthorizedDirectToolCall
...
class TestCryptographicSignatures:
    ...
    async def test_ed25519_signature_creation_and_verification(self, setup_crypto):
        ...
        @TrustedTool("crypto_test", algorithm=SignatureAlgorithm.ED25519, registry=registry)
    ...
    async def test_signature_tampering_detection(self, setup_crypto):
        ...
        assert "hash" in verification.error_message.lower() or "integrity" in verification.error_message.lower()
...
class TestHallucinationDetection:
    ...
    def test_detect_fake_tool_claims(self, hallucination_detector: HallucinationDetector):
        ...
        for response in fake_responses:
            ...
            assert not validation.valid, f"Failed to detect response without signature: {response}"
            assert len(validation.hallucinations) > 0
            
            print(f"   ✅ Detected response without signature: {response[:50]}...")

    def test_legitimate_conversation_allowed(self, hallucination_detector: HallucinationDetector):
...
class TestAutomaticInterception:
    ...
    def test_direct_call_blocked_in_strict_mode(self, interception_setup):
        ...
        interceptor = enable_automatic_enforcement(enforcer, strict_mode=True)
        
        try:
            # Direct call should be blocked
            with pytest.raises(UnauthorizedDirectToolCall):
                result = intercepted_tool("direct call")
...
class TestTrustLevels:
    ...
    async def trust_levels_setup(self):
        ...
        @TrustedTool("critical_trust", trust_level=TrustLevel.CRITICAL, registry=registry)
...
class TestIntegrationScenarios:
    ...
    async def test_end_to_end_verified_workflow(self, full_system_setup):
        ...
        payment_exec = enforcer.execute_tool("payment_api", {"amount": 100.0, "recipient": "user@example.com"})

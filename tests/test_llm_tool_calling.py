...
class AIAgent:
    ...
    async def chat(self, message: str) -> str:
        ...
        elif any(keyword in message.lower() for keyword in ["payment", "send money", "send $", "transfer", "pay"]):
            response = f"I'll help you with the payment. Let me use the payment system."
            # AI decides to call payment tool  
            payment_result = await self._call_payment_tool(100.0, "USD", "friend@example.com")
        ...
        elif any(keyword in message.lower() for keyword in ["analyze data", "analyze", "data analysis", "statistics"]):
    ...
    async def _call_payment_tool(self, amount: float, currency: str, recipient: str) -> Dict[str, Any]:
        """AI calls payment tool - this gets signed!"""
        print(f"  🛠️  AI calling payment_tool for ${amount} {currency}")
        result = await payment_processor(amount, currency, recipient, verify_response=False)
        
        self.tool_calls_made.append({
            "tool": "payment_processor", 
            "args": {"amount": amount, "currency": currency, "recipient": recipient},
            "result": result.data,
            "signature": result.signature.signature[:20] + "...",
            "verified": result.is_verified
        })
...
async def payment_processor(amount: float, currency: str, recipient: str) -> Dict[str, Any]:
...
class LLMToolCallingTests:
    ...
    async def test_conversational_ai_without_tools(self):
        ...
        assert len(agent.tool_calls_made) == 0, "Regular conversation should not trigger tools"
    ...
    async def test_ai_weather_tool_calling(self):
        ...
        assert len(agent.tool_calls_made) == 1, "Weather query should trigger exactly 1 tool call"
    ...
    async def test_ai_payment_tool_calling(self):
        ...
        assert len(agent.tool_calls_made) == 1, "Payment request should trigger exactly 1 tool call"
    ...
    async def test_ai_analytics_tool_calling(self):
        ...
        assert len(agent.tool_calls_made) == 1, "Data analysis should trigger analytics tool"
    ...
    async def test_multi_tool_conversation(self):
        ...
        assert len(agent.tool_calls_made) == 3, "Should have 3 tool calls from 4 conversations"
        ...
        for i, call in enumerate(agent.tool_calls_made):
            ...
            print(f"   ✅ Tool {i+1}: {call['tool']} - Signed: {call['signature'] is not None}")
    ...
    async def test_concurrent_ai_agents(self):
        ...
        for i, agent in enumerate(agents):
            ...
            assert agent.tool_calls_made[0]["signature"] is not None, f"Agent {i} tool call should be signed"
    ...
    async def test_tool_audit_trail(self):
        ...
        signed_calls = sum(1 for call in all_tool_calls if call["signature"] is not None)
    ...
    def print_final_results(self):
        ...
        total_signed = sum(
            len([call for call in agent.tool_calls_made if call["signature"] is not None]) 
            for agent in self.agents
        )

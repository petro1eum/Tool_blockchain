        ...
        try:
            ...
            return json.dumps({
                ...
                "verification": {
                    ...
                    "signature": execution.signature[:16] + "..." if execution.signature else None
                }
            })
...
class ToolEnforcementCallbackHandler(BaseCallbackHandler if LANGCHAIN_AVAILABLE else object):
    ...
    def on_agent_finish(self, finish: "AgentFinish", **kwargs: Any) -> Any:
        ...
        if isinstance(output, str):
            verified_response, proofs, unverified = self.verifier.verify_response(output)
...
class EnforcedAgentExecutor(AgentExecutor if LANGCHAIN_AVAILABLE else object):
        ...
        **kwargs
        ...
        for tool in tools:
            if hasattr(tool, 'name') and tool.name in enforcer.registered_tools:
                ...
                if strict_mode:
                    print(f"⚠️ Warning: Tool '{getattr(tool, 'name', 'unknown')}' not registered with enforcer")
    ...
    def run(self, inputs: Union[str, Dict[str, Any]], **kwargs) -> Any:
        ...
        if isinstance(result, str):
            verified_response, proofs, unverified = self.verifier.verify_response(result)

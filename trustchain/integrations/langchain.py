...
class TrustedLangChainTool(BaseTool if LANGCHAIN_AVAILABLE else object):
    ...
    def __init__(
        self, trusted_tool: BaseTrustedTool, signature_engine: SignatureEngine, **kwargs
    ):
        ...
        super().__init__(
            ...
            or f"Verified tool: {trusted_tool.tool_id}",
    ...
    def _run(self, *args, **kwargs) -> str:
        ...
        try:
            ...
            signed_response = SignedResponse(
                ...
                data=result,
                signature=self.signature_engine.sign_data(
                    result, self.trusted_tool.tool_id
                ),
            )

            # Register with hallucination detector if available
            if hasattr(self, "_hallucination_detector"):
                self._hallucination_detector.verification_registry.register_verified_result(
                    signed_response
                )
...
class VerifiedAgentCallbackHandler(
...
def create_verified_agent_executor(
    ...
    **kwargs,
    ...
    langchain_tools = [
        make_langchain_tool(tool, signature_engine) for tool in trusted_tools
    ]
...
def wrap_existing_tool(
    ...
    return TrustedLangChainTool(
        trusted_tool=wrapped_tool, signature_engine=signature_engine
    )

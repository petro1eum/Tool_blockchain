...
class KeyPairSigner(Signer):
    ...
    def __init__(self, key_pair: KeyPair, signer_id: str):
        ...
        if not key_pair.has_private_key:
            raise CryptoError("Key pair must have private key for signing", 
                            algorithm=key_pair.algorithm.value, operation="signer_init")
...
class MultiSigner(Signer):
    ...
    def get_public_key_id(self) -> str:
        ...
        return f"multisig-{self.threshold}-of-{len(self.signers)}-{'-'.join(signer_ids)}"
...
class SignatureEngine:
    ...
    def create_signer(self, signer_id: str, algorithm: SignatureAlgorithm, **kwargs) -> Signer:
    ...
    def batch_verify(
        self, 
        signed_responses: list[SignedResponse],
        verifier_id: Optional[str] = None
    ...
    def get_signer_public_key(self, signer_id: str, format: str = "base64") -> Optional[str]:

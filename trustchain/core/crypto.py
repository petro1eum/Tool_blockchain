...
class KeyPair(ABC):
    ...
    @abstractmethod
    def export_private_key(self, format: str = "pem", password: Optional[str] = None) -> bytes:
    ...
    @abstractmethod
    def import_private_key(self, key_data: bytes, password: Optional[str] = None) -> None:
...
class Ed25519KeyPair(KeyPair):
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            raise CryptoError(f"Failed to generate Ed25519 key pair: {e}", 
                            algorithm="Ed25519", operation="generate")
    
    def sign(self, data: bytes) -> bytes:
        """Sign data with Ed25519 private key."""
        if not self.has_private_key:
            raise CryptoError("Private key not available for signing", 
                            algorithm="Ed25519", operation="sign")
            ...
            if public_key:
                ...
                if not self.has_public_key:
                    raise CryptoError("Public key not available for verification", 
                                    algorithm="Ed25519", operation="verify")
    ...
    def export_private_key(self, format: str = "pem", password: Optional[str] = None) -> bytes:
        ...
        if not self.has_private_key:
            raise CryptoError("Private key not available", 
                            algorithm="Ed25519", operation="export_private")
        ...
        try:
            if format.lower() == "raw":
                ...
                raise CryptoError(f"Unsupported export format: {format}", 
                                algorithm="Ed25519", operation="export_private")
        except Exception as e:
            raise CryptoError(f"Failed to export private key: {e}", 
                            algorithm="Ed25519", operation="export_private")
    
    def export_public_key(self, format: str = "pem") -> bytes:
        """Export Ed25519 public key."""
        if not self.has_public_key:
            raise CryptoError("Public key not available", 
                            algorithm="Ed25519", operation="export_public")
        ...
        try:
            if len(key_data) == 32:  # Raw private key
                ...
                if not isinstance(crypto_private_key, ed25519.Ed25519PrivateKey):
                    raise CryptoError("Key is not Ed25519", 
                                    algorithm="Ed25519", operation="import_private")
                ...
                try:
                    ...
                    raise CryptoError("Unsupported private key format", 
                                    algorithm="Ed25519", operation="import_private")
            ...
            if isinstance(e, CryptoError):
                raise
            raise CryptoError(f"Failed to import private key: {e}", 
                            algorithm="Ed25519", operation="import_private")
        ...
        try:
            if len(key_data) == 32:  # Raw public key
                ...
                if not isinstance(crypto_public_key, ed25519.Ed25519PublicKey):
                    raise CryptoError("Key is not Ed25519", 
                                    algorithm="Ed25519", operation="import_public")
...
class RSAKeyPair(KeyPair):
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            raise CryptoError(f"Failed to generate RSA key pair: {e}", 
                            algorithm="RSA-PSS", operation="generate")
    
    def sign(self, data: bytes) -> bytes:
        """Sign data with RSA-PSS."""
        if not self.has_private_key:
            raise CryptoError("Private key not available for signing", 
                            algorithm="RSA-PSS", operation="sign")
    ...
    def verify(self, data: bytes, signature: bytes, public_key: Optional[bytes] = None) -> bool:
        ...
        try:
            if public_key:
                ...
                if not self.has_public_key:
                    raise CryptoError("Public key not available for verification", 
                                    algorithm="RSA-PSS", operation="verify")
                verify_key = self._public_key
    ...
    def export_private_key(self, format: str = "pem", password: Optional[str] = None) -> bytes:
        ...
        if not self.has_private_key:
            raise CryptoError("Private key not available", 
                            algorithm="RSA-PSS", operation="export_private")
    ...
    def export_public_key(self, format: str = "pem") -> bytes:
        ...
        if not self.has_public_key:
            raise CryptoError("Public key not available", 
                            algorithm="RSA-PSS", operation="export_public")
    ...
    def import_private_key(self, key_data: bytes, password: Optional[str] = None) -> None:
        ...
        try:
            ...
            if not isinstance(self._private_key, rsa.RSAPrivateKey):
                raise CryptoError("Key is not RSA", 
                                algorithm="RSA-PSS", operation="import_private")
    ...
    def import_public_key(self, key_data: bytes) -> None:
        ...
        try:
            ...
            if not isinstance(self._public_key, rsa.RSAPublicKey):
                raise CryptoError("Key is not RSA", 
                                algorithm="RSA-PSS", operation="import_public")
...
class CryptoEngine:
        ...
        **kwargs
        ...
        if algorithm == SignatureAlgorithm.ED25519:
            ...
            raise CryptoError(f"Unsupported algorithm: {algorithm}", 
                            algorithm=algorithm.value, operation="create_key_pair")
    ...
    def import_key_pair(
        ...
        if algorithm == SignatureAlgorithm.ED25519:
            ...
            raise CryptoError(f"Unsupported algorithm: {algorithm}", 
                            algorithm=algorithm.value, operation="import_key_pair")
    ...
    def hash_data(self, data: Union[str, bytes, Dict[str, Any]], algorithm: str = "sha256") -> str:
        ...
        if algorithm.lower() == "sha256":
            ...
            raise CryptoError(f"Unsupported hash algorithm: {algorithm}", 
                            algorithm=algorithm, operation="hash")

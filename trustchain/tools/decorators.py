...
def TrustedTool(
    ...
    def decorator(func: Callable) -> Callable:
        print(
            f"🔧 [DECORATOR DEBUG] Creating TrustedTool '{tool_id}' for function {func.__name__}",
            flush=True,
        )
        ...
        if multi_sig:
            ...
            print(
                f"🔧 [DECORATOR DEBUG] Created MultiSigFunctionTool for '{tool_id}'",
                flush=True,
            )
        else:
            print(
                f"🔧 [DECORATOR DEBUG] Creating FunctionTrustedTool for '{tool_id}'",
                flush=True,
            )
            ...
            print(
                f"🔧 [DECORATOR DEBUG] FunctionTrustedTool created for '{tool_id}'",
                flush=True,
            )
        ...
        async def wrapper(*args, **kwargs):
            print(
                f"🔧 [WRAPPER DEBUG] Calling wrapped tool '{tool_id}' with args={args}, kwargs={kwargs}",
                flush=True,
            )
            ...
            print(
                f"🔧 [WRAPPER DEBUG] About to call tool '{tool_id}' with verify_response={verify_response}",
                flush=True,
            )
            # Call the tool
            return await tool(

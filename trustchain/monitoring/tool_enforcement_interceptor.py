...
class ToolCallInterceptor:
    ...
    def enable_global_interception(self) -> None:
        ...
        print(
            f"✅ [INTERCEPTOR] Global interception enabled for {len(self.intercepted_tools)} tools"
        )
    ...
    def _handle_tool_call(
        self, tool: BaseTrustedTool, original_call: Callable, *args, **kwargs
    ) -> Any:
        ...
        if self._is_call_authorized():
            ...
            if self.strict_mode:
                ...
                print(
                    f"⚠️ [INTERCEPTOR] WARNING: Direct call to {tool.tool_id} detected but allowed in non-strict mode"
                )
    ...
    def _is_call_authorized(self) -> bool:
        ...
        try:
            ...
            for _i in range(10):  # Check up to 10 frames up
                ...
                if (
                    "tool_enforcement.py" in filename
                    and function_name == "execute_tool"
                ):
                    return True
    ...
    def _get_call_stack(self) -> str:
        ...
        try:
            ...
            for _i in range(5):  # Show last 5 frames
                if not frame:
                    break
    ...
    def _patch_trusted_tool_decorator(self) -> None:
        ...
        interceptor_ref = self  # Store reference to the interceptor
        ...
        def patched_init(self, tool_id: str, **kwargs):
            ...
            if hasattr(self, "_trustchain_tool") and self._trustchain_tool:
                interceptor_ref._intercept_tool(self._trustchain_tool)
...
def enable_automatic_enforcement(
    enforcer: ToolExecutionEnforcer, strict_mode: bool = True
) -> ToolCallInterceptor:

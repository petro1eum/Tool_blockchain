...
class MemoryRegistry(TrustRegistry):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "memory_usage": {"keys": len(self._keys), "tools": len(self._tools)},
            }
    ...
    async def search_keys(self, query: Dict[str, Any]) -> List[KeyMetadata]:
        ...
        with self._lock:
            ...
            results.sort(key=lambda k: (k.usage_count, k.valid_from), reverse=True)
        ...
        with self._lock:
            for key_metadata in keys:
                try:
                    ...
                    errors.append({"key_id": key_metadata.key_id, "error": str(e)})
                    error_count += 1

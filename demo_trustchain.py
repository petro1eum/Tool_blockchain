...
async def get_weather(location: str) -> Dict[str, Any]:
    ...
    result = weather_data.get(location, {"temp": 20, "humidity": 60, "condition": "Unknown"})
...
async def main():
    ...
    try:
        ...
        print(f"   🧮 Calculation: {calc_response.data['operands'][0]} × {calc_response.data['operands'][1]} = {calc_response.data['result']:.2f}")
        ...
        print(f"       Range: {data_response.data['min']} - {data_response.data['max']}")
        print(f"   🔐 Signature Format: {data_response.signature.signature_format.value}")
        print()
        ...
        print(f"   ⚡ Processed 10 calculations in {(end_time - start_time)*1000:.2f}ms")

...
async def get_latest_news(category: str = "tech") -> Dict[str, Any]:
    ...
    filtered_news = [
        ...
        if category.lower() in article.title.lower() or category.lower() in article.content.lower()
    ]
...
class HallucinationDemo:
    ...
    def verify_response_authenticity(self, response: Any) -> Dict[str, Any]:
        ...
        if isinstance(response, SignedResponse):
            verification.update({
                "has_signature": hasattr(response, 'signature') and response.signature is not None,

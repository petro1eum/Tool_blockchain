    ...
    "fields": [
        ...
        {"name": "context", "type": {
            ...
            "fields": [
                ...
                {"name": "parent_request_id", "type": ["null", "string"], "default": None}
            ]
        }},
            ...
            try:
                ...
                if cached:
                    ...
                    self.signature_cache.put(
                        cache_key.encode(),
                        json.dumps(verification_result).encode()
                    )
                ...
                await producer.send(
                    ...
                    headers=[
                        ("verified", b"true" if verification_result["valid"] else b"false"),
...
class KafkaNonceRegistry:
    ...
    async def check_and_register_nonce(
        self,
        nonce: str,
        request_id: str
    ) -> bool:
        ...
        for message in self.consumer:
            ...
            if not result["valid"]:
                ...
                if failure_rate > 0.01:  # 1% порог
                    self._send_alert(
                        f"High signature failure rate: {failure_rate:.2%}"
                    )
            
            # Метрики по инструментам
            tool_id = result["tool_id"]
            if tool_id not in self.metrics["tools"]:
                self.metrics["tools"][tool_id] = {
                    "total": 0,
                    "failed": 0
                }
